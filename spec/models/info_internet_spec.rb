...
RSpec.describe InfoInternet, :type => :model do
  it "is valid with a entity, ip address, speed, macs, modem serial, modem brand, subnet mask
  dns, gateway, node, password wifi, equipment an user" do
    info_internet = InfoInternet.new(entidad_id: 10, direccionip: '127.0.0.1', velocidad: 3, mac1: '168.0.0.125', 
  ...
  it "is invalid without a entity" do
    info_internet = InfoInternet.new(entidad_id: nil)
    info_internet.valid?
    expect(info_internet.errors[:entidad]).to include("can't be blank")
  end
